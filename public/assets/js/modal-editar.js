document.addEventListener('DOMContentLoaded', function() {
    ...
    editarUsuarioModal.addEventListener('show.bs.modal', function(event) {
        ...
        // Obtener datos de la fila (ajusta los índices según tu estructura)
        const celdas = row.querySelectorAll('td');
        
        const nombre = celdas[0].innerText;
        const apellidos = celdas[1].innerText;
        const tipoDocumento = celdas[2].innerText || celdas[2].getAttribute('data-tipo-documento');
    ...
    function setSelectValue(selectId, value) {
        ...
        // Si el valor está vacío, seleccionar la primera opción (o la opción vacía si existe)
        if (!value || value === '---') {
